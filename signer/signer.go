	...
	for _, val := range csrv.Extensions {
		...
		if val.Id.Equal(asn1.ObjectIdentifier{2, 5, 29, 19}) {
			...
			if rest, err = asn1.Unmarshal(val.Value, &constraints); err != nil {
				...
				return nil, cferr.Wrap(cferr.CSRError, cferr.ParseFailed, errors.New("sm2: trailing data after X.509 BasicConstraints"))
			}
...
func FillTemplate(template *sm2.Certificate, defaultProfile, profile *config.SigningProfile, notBefore time.Time, notAfter time.Time) error {
	...
	for _, val := range eku {
		template.ExtKeyUsage = append(template.ExtKeyUsage, sm2.ExtKeyUsage(val))
	}
...
// certificate, based on the input config. Go's sm2 library allows setting
