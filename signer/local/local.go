...
func (s *Signer) sign(template *sm2.Certificate, profile *config.SigningProfile, notBefore time.Time, notAfter time.Time) (cert []byte, err error) {
...
func (s *Signer) Sign(req signer.SignRequest) (cert []byte, err error) {
	...
	// In prior versions of Go, sm2.CreateCertificate updated the
	// AuthorityKeyId of certTBS.
	//parsedCert, _ := sm2.ParseCertificate(signedCert)

	if s.dbAccessor != nil {
		var certRecord = certdb.CertificateRecord{
			Serial: certTBS.SerialNumber.String(),
			// this relies on the specific behavior of sm2.CreateCertificate
